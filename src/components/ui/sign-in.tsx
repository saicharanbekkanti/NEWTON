...
export const SignInPage: React.FC<SignInPageProps> = ({
    ...
    const activeTitle = title || (isSignIn ? <span className="font-light text-foreground tracking-tighter">Welcome</span> : <span className="font-light text-foreground tracking-tighter">Create Account</span>);
    const activeDescription = description || (isSignIn ? "Access your account and continue your journey with us" : "Join our community and start your journey today");

    return (
        <div className="h-[100dvh] flex flex-col md:flex-row font-geist w-[100dvw] overflow-x-hidden">
            {/* Left column: form */}
            <section className="flex-1 flex items-center justify-center p-8">
                <div className="w-full max-w-md" key={mode}>
                    <div className="flex flex-col gap-6">
                        <h1 className="animate-element animate-delay-100 text-4xl md:text-5xl font-semibold leading-tight ">{activeTitle}</h1>
                        <p className="animate-element animate-delay-200 text-muted-foreground ">{activeDescription}</p>

                        <form className="space-y-5" onSubmit={isSignIn ? onSignIn : onSignUp}>
                            ...
                            {isSignIn && (
                                <div className="animate-element animate-delay-500 flex items-center justify-between text-sm ">
                                    <label className="flex items-center gap-3 cursor-pointer">
                                        <input type="checkbox" name="rememberMe" className="w-4 h-4 rounded border-border focus:ring-violet-500" />
                        ...
                        <div className={`animate-element ${isSignIn ? 'animate-delay-700' : 'animate-delay-800'} relative flex items-center justify-center `}>
                        ...
                        <p className={`animate-element ${isSignIn ? 'animate-delay-900' : 'animate-delay-1000'} text-center text-sm text-muted-foreground `}>
                            {isSignIn ? (
                                <>New to our platform? <a href="#" onClick={(e) => { e.preventDefault(); setMode('signup'); }} className="text-violet-400 hover:underline transition-colors">Create Account</a></>
                            ) : (
                                <>Already have an account? <a href="#" onClick={(e) => { e.preventDefault(); setMode('signin'); }} className="text-violet-400 hover:underline transition-colors">Sign In</a></>
                            )}
                        </p>
                    </div>
                </div>
